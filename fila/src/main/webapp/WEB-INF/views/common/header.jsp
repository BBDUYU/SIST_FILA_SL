...
<header id="header">
		...
		<ul class="gnb">
			<c:forEach items="${list}" var="d1">
				<c:if test="${d1.depth eq 1}">
					<li><a
						href="${pageContext.request.contextPath}/product/list.htm?category=${d1.categoryId}">${d1.name}</a>
									...
									<c:forEach items="${list}" var="d2">
										<c:if test="${d2.depth eq 2 && d2.parentId eq d1.categoryId}">
											<div class="category-group">
												<a
													href="${pageContext.request.contextPath}/product/list.htm?category=${d2.categoryId}"
													class="link-tit">${d2.name}</a>
												<div>
													<ul>
														<li><a
															href="${pageContext.request.contextPath}/product/list.htm?category=${d2.categoryId}">전체보기</a></li>
														<c:forEach items="${list}" var="d3">
															<c:if
																test="${d3.depth eq 3 && d3.parentId eq d2.categoryId}">
																<li><a
																	href="${pageContext.request.contextPath}/product/list.htm?category=${d3.categoryId}">${d3.name}</a></li>
															</c:if>
														</c:forEach>
													</ul>
												</div>
	...
	<div class="util">
		<div class="util-store">
			<a href="${pageContext.request.contextPath}/noticeList.htm"
				class="store__btn"> store </a>
		</div>

		<div class="util-search">
			<button type="button" class="search-open__btn">search</button>
			<form action="${pageContext.request.contextPath}/product/list.htm"
				name="searchForm2" method="get"
										...
										<c:choose>
											<c:when test="${not empty popularKeywords}">
												<c:forEach var="sDto" items="${popularKeywords}">
													<li><a
														href="${pageContext.request.contextPath}/product/list.htm?searchItem=${fn:escapeXml(sDto.keyword)}">${sDto.keyword}</a></li>
												</c:forEach>
											</c:when>
											<c:otherwise>
												<li>검색 기록이 없습니다.</li>
												...
												<c:forEach var="rDto" items="${recommendKeywords}">
													<li><c:choose>
															...
															<c:when test="${not empty rDto.slug}">
																<a
																	href="${pageContext.request.contextPath}/event/event.htm?slug=${rDto.slug}">${rDto.eventName}</a>
															</c:when>
														</c:choose></li>
												</c:forEach>
											</c:when>
																	...
																	<div class="info">
																		<a
																			...
																			<div class="price">
																				<p class="sale">
																					<fmt:formatNumber
																						value="${pDto.price * (100 - pDto.discountRate) / 100}"
																						type="number" />
																					원
																				</p>
																			</div>
		...
		<div class="util-cart">
			<button type="button" class="cart__btn" id="cart_cnt"
				onclick="location.href='${pageContext.request.contextPath}/pay/cart.htm';">cart</button>
		</div>
	</div>
</header>