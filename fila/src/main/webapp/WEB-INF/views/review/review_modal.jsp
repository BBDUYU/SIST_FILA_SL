...
<c:set var="prodImg" value="${product.imageUrl}" />
<c:if test="${fn:contains(prodImg, 'C:\\fila_upload')}">
    <c:set var="prodImg" value="${fn:replace(prodImg, 'C:\\\\fila_upload', '/upload')}" />
    <c:set var="prodImg" value="${fn:replace(prodImg, 'C:\\fila_upload', '/upload')}" />
    <c:set var="prodImg" value="${fn:replace(prodImg, '\\\\', '/')}" />
    <c:set var="prodImg" value="${fn:replace(prodImg, '\\', '/')}" />
</c:if>
<%-- 이미지가 없으면 기본 이미지 사용 --%>
<c:if test="${empty prodImg}">
    <c:set var="prodImg" value="//filacdn.styleship.com/filaproduct2/data/productimages/a/1/FS253IP02F003_734.jpg" />
</c:if>
...
<div class="common__layer _review" id="reviewModal" style="display:none;">
    ...
    <div class="inner">
        ...
        <div id="reviewListView" style="height:100%; display:flex; flex-direction:column;">
            <div class="head">
                <div class="goods-info">
                    <div class="photo">
                        <%-- [수정] 위에서 보정한 변수(prodImg) 사용 --%>
                        <img src="${prodImg}" alt="상품이미지">
                    </div>
                    <div class="info">
        ...
        <div id="reviewWriteView" style="height:100%; display:none; flex-direction:column;">
            <div class="head">
                <div class="goods-info">
                    <%-- [수정] 작성 화면의 썸네일도 보정된 prodImg 사용 --%>
                    <div class="photo"><img src="${prodImg}" alt="상품이미지"></div>
...
function searchReviews() {
    ...
    // 디버깅 로그
    console.log("리스트 요청: " + productId + ", " + finalSort);
    ...
    $.ajax({
        ...
        success: function(htmlFragment) {
            ...
            if (typeof checkReviewHeightLocal === 'function') {
...
function handleSearchKey(e) { if (e.keyCode === 13) searchReviews(); }
...
function toggleReviewText(btn) {
    ...
    if ($textDiv.hasClass("full")) { $textDiv.removeClass("full"); $(btn).text("리뷰 더보기"); } 
    else { $textDiv.addClass("full"); $(btn).text("리뷰 접기"); }
}
...
function openReviewModal() {
    ...
    switchToList();
    searchReviews(); // 열릴 때 리스트 로드
    setTimeout(function() { checkReviewHeight(); }, 50);
}
function closeReviewModal() { $('#reviewModal').fadeOut(200); $('body').removeClass('no-scroll'); resetWriteForm(); }
function switchToList() { $('#reviewWriteView').hide(); $('#reviewListView').css('display', 'flex'); }
function switchToWrite() { resetWriteForm(); $('#reviewListView').hide(); $('#reviewWriteView').css('display', 'flex'); }
function resetWriteForm() {
    sel_files = []; $("#imgPreviewBox").empty(); $("#reviewFiles").val(""); $("textarea[name='content']").val("");
    setRating(5); $("#scoreText").text("아주 좋아요"); $(".write-star-svg").css("fill", "#003F96"); $("#fileCount").text("(0/4)");
}
function setRating(rating) {
    $("#reviewScore").val(rating);
    $(".write-star-svg").each(function() {
        var starIndex = $(this).data("index");
        if (starIndex <= rating) $(this).css("fill", "#003F96"); else $(this).css("fill", "#ddd");
    });
    var scoreTexts = ["", "별로예요", "그냥 그래요", "보통이에요", "맘에 들어요", "아주 좋아요"];
    $("#scoreText").text(scoreTexts[rating]);
}
function handleImgPreview(e) {
    var files = e.files; var filesArr = Array.prototype.slice.call(files);
    var totalCnt = sel_files.length + filesArr.length;
    if (totalCnt > 4) { alert("사진은 최대 4장까지만 등록 가능합니다."); $(e).val(""); return; }
    filesArr.forEach(function(f) {
        if (!f.type.match("image.*")) return;
        sel_files.push(f); 
        var reader = new FileReader();
        reader.onload = function(e) {
            var html = '<div class="img-thumb" style="position:relative; width:80px; height:80px; margin-right:10px; margin-bottom:10px; display:inline-block;">' +
                '<img src="' + e.target.result + '" style="width:100%; height:100%; object-fit:cover; border-radius:4px; border:1px solid #ddd;">' +
                '<button type="button" onclick="deleteSelImage(this, \'' + f.name + '\')" style="position:absolute; top:-5px; right:-5px; background:#000; color:#fff; border:0; width:20px; height:20px; border-radius:50%; font-size:12px; cursor:pointer; display:flex; align-items:center; justify-content:center;">X</button></div>';
            $("#imgPreviewBox").append(html);
        }
        reader.readAsDataURL(f);
    });
    updateFileCount(); $(e).val(""); 
}
function deleteSelImage(btn, fileName) {
    sel_files = sel_files.filter(function(f) { return f.name !== fileName; });
    $(btn).parent().remove(); updateFileCount();
}
function updateFileCount() { $("#fileCount").text("(" + sel_files.length + "/4)"); }
function handleLike(btn, reviewId, type) {
    // ★★★ [수정] 따옴표 필수 ★★★
    var isLogin = "${sessionScope.auth}";
    ...
    $.ajax({
        ...
        success: function(res) {
            if (res.trim() === "1") { alert("반영되었습니다."); var countSpan = $(btn).find("span"); countSpan.text(parseInt(countSpan.text()) + 1); $(btn).find("svg").css("fill", "#003F96"); $(btn).css("color", "#003F96"); }
            else { alert("이미 평가하셨거나 오류가 발생했습니다."); }
        },
